...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        # Adapted by Anas for experience replay
        max_term = self.qnetwork_target(torch.tensor([next_state])).detach().max(dim=1, keepdim=True)[0]
        abs_td_error = abs(reward + GAMMA * max_term - \
                           self.qnetwork_local(torch.tensor([state])).gather(dim=1, index=torch.tensor([[action]])))
    ...
    def learn(self, experiences, gamma):
        ...
        # self.running_loss.append(loss.norm().item())
        # print(f"\rlatest running loss: {self.running_loss[-1]}", end="")
